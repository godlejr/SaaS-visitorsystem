...
def index():
    ...
    if request.method == 'POST':
        ...
        if visit_sdate != "" and visit_edate != "":
            vcApplyMasterQuery = vcApplyMasterQuery.filter(Vcapplymaster.visit_sdate >= visit_sdate,
                                                           Vcapplymaster.visit_edate <= visit_edate)

        # print("=======Query=========")
        # print(vcApplyMasterQuery)

        # 회사명 검색도 될 수 있도록 설정 필요
        applyList = vcApplyMasterQuery.filter(
            Vcapplymaster.visit_purpose.like("%" + form.visit_purpose.data + "%")).all()
