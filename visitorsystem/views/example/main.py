...
main_example = Blueprint('main_example', __name__)
mail = Mail()


@main_example.context_processor
...
@main_example.route('/login', methods=['GET', 'POST'])
def login():
        ...
        if form.validate():
            ...
            if user:
                if not check_password_hash(user.password, form.password.data):
                    ...
                    return redirect(request.args.get("next") or url_for('main_example.index'))
            else:
...
def logout():
    session.clear()
    return redirect(url_for('main_example.index'))


@main_example.route('/join', methods=['GET', 'POST'])
def join():
    ...
    if request.method == 'POST':
        ...
        if form.validate():
            user = User()
            if User.query.filter_by(email=request.form.get('email')).first():
                return redirect(url_for('main_example.join'))
            ...
            flash('가입을 축하합니다.')
            return redirect(url_for('main_example.login'))

    return render_template(current_app.config['TEMPLATE_THEME'] + '/main_example/join.html', form=form)


@main_example.route('/privacy', methods=['GET', 'POST'])
def privacy():
    return render_template(current_app.config['TEMPLATE_THEME'] + '/main_example/privacy.html')


@main_example.route('/agreement', methods=['GET', 'POST'])
def agreement():
    return render_template(current_app.config['TEMPLATE_THEME'] + '/main_example/agreement.html')


@main_example.route('/confirm_key', methods=['GET', 'POST'])
def confirm_key():
    if current_app.redis.get(request.args.get('key')):
        return redirect(url_for('main_example.edit_password', key=request.args.get('key')))
    return redirect(url_for('main_example.index'))


@main_example.route('/edit_password/<key>', methods=['GET', 'POST'])
def edit_password(key):
    ...
    return render_template(current_app.config['TEMPLATE_THEME'] + '/main_example/edit_password.html', form=form)


@main_example.route('/password', methods=['GET', 'POST'])
def password():
    print('------------ss-------')
    print(User.query)
    print('-------------------')
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('main_example.login'))
    return render_template(current_app.config['TEMPLATE_THEME'] + '/main_example/password.html', form=form)


@main_example.route('/facebook_login', methods=['POST'])
def facebook_login():
